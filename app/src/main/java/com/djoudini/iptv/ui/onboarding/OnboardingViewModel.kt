...
class OnboardingViewModel @Inject constructor(
    ...
    fun loginXtream(host: String, user: String, pass: String) {
        viewModelScope.launch {
            ...
            when (val result = iptvRepository.loginXtream(host, user, pass)) {
                is Result.Success -> {
                    val providerId = result.data.providerId
                    _uiState.update { it.copy(isLoading = false, currentStep = OnboardingStep.CategoryFilter, providerId = providerId) }
    ...
    private fun loadCategories(providerId: Long) {
        viewModelScope.launch {
            ...
            iptvRepository.loadCategoriesForOnboarding(providerId)
